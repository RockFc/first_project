...
public:
    ...
    typename State<T>::ValueType Wait(const std::chrono::milliseconds& timeout
                                      = std::chrono::milliseconds(24 * 3600 * 1000))
    {
        ...
        this->Then(
            [&value, &ready, wcond = std::weak_ptr<std::condition_variable>(cond),
        ...
        bool                         success = cond->wait_for(waiter, timeout,
                                                              [&ready]()
                                                              {
                                          return ready;
                                      });
    ...
    typename std::enable_if<IsFuture<SHIT>::value, SHIT>::type Unwrap()
    {
        ...
        {
            _SetCallback(
                [pm = std::move(prom)](typename TryWrapper<SHIT>::Type&& innerFuture) mutable
                {
                    try
                    {
                        SHIT future = std::move(innerFuture);
                        future._SetCallback(
                            [pm = std::move(pm)](typename TryWrapper<InnerType>::Type&& t) mutable
                            {
    ...
    _ThenImpl(Scheduler* sched, F&& f, ResultOfWrapper<F, Args...>)
    {
        ...
        else if (state_->progress_ == Progress::Done)
        {
            ...
            {
                sched->Schedule(
                    [t = std::move(t), f = std::forward<FuncType>(f), pm = std::move(pm)]() mutable
                    {
        ...
        else
        {
            ...
            nextFuture._SetOnTimeout(
                [weak_parent = std::weak_ptr<State<T>>(state_)](TimeoutCallback&& cb)
                {
            ...
            _SetCallback(
                 ...
                 prom = std::move(pm)](typename TryWrapper<T>::Type&& t) mutable
                {
                    ...
                    {
                        sched->Schedule(
                            [func = std::move(func), t = std::move(t),
    ...
    _ThenImpl(Scheduler* sched, F&& f, ResultOfWrapper<F, Args...>)
    {
        ...
        else if (state_->progress_ == Progress::Done)
        {
            ...
            auto cb =
                [res = std::move(t), f = std::forward<FuncType>(f), prom = std::move(pm)]() mutable
            {
                // because func return another future: innerFuture, when innerFuture is done,
                // nextFuture can be done
                decltype(f(res.template Get<Args>()...)) innerFuture;
                if (res.HasException())
                {
                    // Failed if Args... is void
                    innerFuture = f(typename TryWrapper<typename std::decay<Args...>::type>::Type(
                        res.Exception()));
                }
                else
                {
                ...
                else
                {
                    innerFuture._SetCallback(
                        [prom = std::move(prom)](typename TryWrapper<FReturnType>::Type&& t) mutable
                        {
                            prom.SetValue(std::move(t));
                        });
                }
        ...
        else
        {
            ...
            // 2. set this future's then callback
            _SetCallback(
                [sched = sched, func = std::forward<FuncType>(f),
                 prom = std::move(pm)](typename TryWrapper<T>::Type&& t) mutable
                {
                    auto cb =
                        [func = std::move(func), t = std::move(t), prom = std::move(prom)]() mutable
                    {
    ...
    {
        scheduler->ScheduleLater(duration,
                                 [state = state_, cb = std::move(f)]() mutable
                                 {
                                     {
...
WhenAll(FT&&... futures)
{
    auto ctx = std::make_shared<
        CollectAllVariadicContext<typename std::decay<FT>::type::InnerType...>>();
...
Future<std::vector<
    typename TryWrapper<typename std::iterator_traits<InputIterator>::value_type::InnerType>::Type>>
WhenAll(InputIterator first, InputIterator last)
{
    using TryT = typename TryWrapper<
...
// When N
template <class InputIterator>
Future<std::vector<std::pair<size_t,
                             typename TryWrapper<typename std::iterator_traits<
...
WhenIfAny(
    ...
    InputIterator last,
    std::function<
        bool(const Try<typename std::iterator_traits<InputIterator>::value_type::InnerType>&)> cond)
{
...
WhenIfN(size_t                                                                                N,
        ...
        InputIterator                                                                         last,
        std::function<bool(
            const Try<typename std::iterator_traits<InputIterator>::value_type::InnerType>&)> cond)
{
