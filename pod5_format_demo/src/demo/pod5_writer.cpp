...
int main(int argc, char** argv)
{
    ...
    // 4. 开始 POD5 写入流程
    {
        ...
        if (pod5_vbz_compress_signal(signal_data.data(), signal_data.size(),
                                     compressed_signal.data(), &compressed_size)
            != POD5_OK)
        {
            std::cerr << "Error: VBZ compression failed: " << pod5_get_error_string() << "\n";
            return EXIT_FAILURE;
        }
        ...
        std::cout << "Compression ratio: " << static_cast<float>(original_size) / compressed_size
                  << "\n";
        ...
        if (pod5_add_reads_data_pre_compressed(file, 1, READ_BATCH_ROW_INFO_VERSION_3, &row_data,
                                               &compressed_data_ptr, &compressed_size_ptr,
                                               &signal_size_ptr, &signal_counts)
            != POD5_OK)
        {
            std::cerr << "Error: Failed to write pre-compressed data: " << pod5_get_error_string()
                      << "\n";
            return EXIT_FAILURE;
        }
        ...
        for (std::size_t batch_index = 0; batch_index < batch_count; ++batch_index)
        {
            ...
            if (pod5_get_read_batch(&batch, file, batch_index) != POD5_OK)
            {
                std::cerr << "Failed to get batch: " << batch_index
                          << "error info: " << pod5_get_error_string() << "\n";
                return EXIT_FAILURE;
            }
            ...
            for (std::size_t row = 0; row < batch_row_count; ++row)
            {
                ...
                std::cout << "======================\n";
                std::cout << "read_data.calibration_offset: " << read_data.calibration_offset
                          << "\n";
                std::cout << "read_data.calibration_scale: " << read_data.calibration_scale << "\n";
                std::cout << "read_data.tracked_scaling_scale: " << read_data.tracked_scaling_scale
                          << "\n";
                std::cout << "read_data.tracked_scaling_shift: " << read_data.tracked_scaling_shift
                          << "\n";
                std::cout << "read_data.predicted_scaling_scale: "
                          << read_data.predicted_scaling_scale << "\n";
                std::cout << "read_data.predicted_scaling_shift: "
                ...
                {
                    std::cerr << "Failed to check size! num_samples:" << read_data.num_samples
                              << ", signal_data.size:" << signal_data.size() << "\n";
                    return EXIT_FAILURE;
                }
