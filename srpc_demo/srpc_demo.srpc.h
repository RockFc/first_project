...
inline Service::Service(): srpc::RPCService("srpc_demo")
{
...
{
	auto *task = this->create_rpc_client_task("/srpc_demo/Echo", std::move(done));
...
inline WFFuture<std::pair<EchoResponse, srpc::RPCSyncContext>> TRPCClient::async_Echo(const EchoRequest *req)
{
	...
	auto *pr = new WFPromise<RESULT>();
	auto fr = pr->get_future();
	auto *task = this->create_rpc_client_task<EchoResponse>("/srpc_demo/Echo", srpc::RPCAsyncFutureCallback<EchoResponse>);
